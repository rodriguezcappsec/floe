...
The completed Phase 13A/13B capabilities share one search row: `Ctrl+F` opens
Quick Filter, the visible mode selector switches to Search Files, and
`Ctrl+Shift+F` opens Search Files directly. The shared entry and Close control
do not merge the engines: Quick Filter narrows loaded entries, while Search
Files uses the bounded filename traversal worker.
...
| Type/extension/MIME filters | `COMPLETE` | 13C | Structured combined predicates; MIME is lazily guessed from the exact path/name without content reads and unknown required metadata excludes rather than broadens results. |
| Size/date/owner/hidden filters | `COMPLETE` | 13C | Fixed size/date presets, no-follow Unix owner resolution, and Current Setting/Include Hidden/Hidden Only policy run on bounded workers. |
| Tag filters | `DEFERRED` | 19 | No tag model exists. |
| Glob/regex/case sensitivity | `COMPLETE` | 13C | Shared Text/Glob/Regex matcher and explicit Match Case serve Quick Filter and Search Files while preserving raw non-UTF-8 fallback. |
...
| Search sorting/grouping | `PLANNED` | 13E | Reuse view policy rather than create separate filesystem actions; Phase 13C retains streamed traversal order. |
...
| Hidden/filter/search mode obvious | `COMPLETE` | 1/13A/13B | Hidden toggle, Text/Glob/Regex filter row, and distinct filename-search row expose visible scope, Search, Stop, Close, progress, and result state. |
