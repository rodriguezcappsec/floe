...
| Owner/permissions/link/path sorting | `COMPLETE` | 20B1A | Explicit current-folder index reads no-follow Unix/link facts off GTK; exact raw paths remain authoritative and unknown link destinations remain last. |
...
| Open as Administrator | `COMPLETE` | 14B | Experimental opt-in read-only view constructs typed local `admin` GFiles through GLib/GIO URI APIs, delegates authentication to GVfs/polkit, pages no-follow metadata, rejects stale generations, keeps a persistent accessible Administrator state, and cancels/returns explicitly. Floe stays UID-stable and privileged resources have no mutation, preview, terminal, archive, launcher, custom-action, clipboard, or plugin path. Privileged mutations and removal of the experimental guard remain separately gated. |
