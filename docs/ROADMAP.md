...
| 18Y2 — Complete Undo and recovery | NEXT | `phase-18y2-complete-undo-recovery` | Operation-specific persistent Undo/Redo and crash recovery for completed local and administrator mutations where reversal can be proven safe. | Requires pre-commit backup/staging semantics, exact identity revalidation, no-overwrite inverse jobs, private bounded journal/history, explicit expiry, partial/uncertain review, no claim for irreversible delete or unsupported backend operations. |
