...
| 22B — Optional XDG FileChooser portal backend | NEXT | `phase-xdg-filechooser-portal` | Optionally serve the freedesktop FileChooser backend contract through the verified Selection Mode UI. | 22A; isolate request handles, parent identifiers, response codes, cancellation, URI/document grants, and service availability. Floe remains independently usable when the portal backend is absent and must not imply sandbox permissions it did not grant. |
