...
| 6U — Replace conflict safety | COMPLETE | `phase-6u-replace-conflict-safety` | Mature Replace and batch-scoped Replace All with recoverable backup/rollback policy. | Verified exact no-follow source/destination revalidation, owner-only capacity-bounded same-filesystem backups, atomic version exchange, pre-commit cancellation/rollback, explicit partial outcomes, durable Undo/Redo, identity-owned expiry cleanup, accessible comparison and second confirmation. Replace All captures fresh identities per compatible conflict and remains scoped to one batch; Protected Folder intersections pause for fresh review. |
...
| 6W — Undo Trash | COMPLETE | `phase-6w-undo-trash` | Durable Undo/Redo for a Floe-owned move to standards-correct local Trash. | Verified exact raw original/payload/metadata paths, no-follow identities, bounded local receipt discovery, no-overwrite restore, 30-day restart-safe history, explicit conflict/partial review, and a fresh receipt after Redo. Successful administrator, remote, ambiguous, unsafe, or oversized Trash operations remain non-undoable; permanent deletion is never undoable. |
...
| 14C — Safe administrator operations | COMPLETE | `phase-14c-privileged-operations` | Explicit typed create, single-file copy/move, rename, Trash/delete, and Unix-mode mutations inside an administrator location without elevating GTK. | Verified separate capacity-one GIO/GVfs service, request-scoped mount operation, exact raw identity, no-follow fingerprint revalidation, no-overwrite flags, accessible confirmation, cancellation/partial-output evidence, close blocking, real-GTK controls, and local-job/shell exclusion. Recursive copy, ownership, ACL/xattr, cross-restart privileged recovery, root-owned native fixture, Niri remain unavailable/unverified. |
...
| 18L — Sandboxed providers | COMPLETE | `phase-23h-security-inspection-suite` | Bubblewrap target-only read, private output/temp, no network/session namespaces, bounded process-group lifecycle. | Production fails closed when Bubblewrap is absent or unusable and never retries unsandboxed; policy and controlled-fixture gates pass, while this host sandbox could not run nested Bubblewrap natively. |
| 18M — Open Safely | DEFERRED | — | Intentionally removed from the current product scope after implementation review. | Compatibility cost and UI/policy surface outweighed its daily-use value; ordinary Open/Open With remain. Revisit only after an explicit product decision. |
| 18N — Suspicious files and optional local ClamAV | COMPLETE | `phase-23h-security-inspection-suite` | Explainable executable/double-extension, MIME, bidi/control findings plus bounded local `clamd` scanning with persisted user-configurable per-file and total-request limits snapshotted into each scan. | No cloud, shell, bundled engine, quarantine, automatic deletion, or malware-free verdict; independent daemon limits and no-signature, detected, changed, not-scanned, limit, error and cancelled remain distinct. Valid pending daemon-limit responses survive early `INSTREAM` socket closure without accepting a partial-stream clean result. |
| 18O — Privacy Inspector | COMPLETE | `phase-23h-security-inspection-suite` | Explicit local JPEG/TIFF EXIF evidence and PNG/WebP metadata-chunk findings in context menu and Properties. | GPS, camera/device, software, creator, timestamps and embedded-thumbnail evidence is format-specific; unsupported/malformed/changed/oversized remain explicit and no exhaustive claim is made. |
| 18P — Metadata sanitization | COMPLETE | `phase-23h-security-inspection-suite` | Preview-confirmed batch JPEG/PNG/WebP no-overwrite sanitized sibling copies. | Sources are never modified; private staged output is verified then atomically published with no-replace semantics; unsupported/failed/cancelled items remain explicit and removal is not an anonymity guarantee. |
...
| 18R — Permission auditor | COMPLETE | `phase-18r-permission-auditor` | Explain risky modes, ownership, ACL/xattr/capability exposure and conservative fixes. | Verified bounded local no-follow 128-path inspection, exact mode/UID/GID/mount evidence, bounded xattr-name and immutable probes, calm findings, symlink/changed/unsupported states, and single-item audited-identity-bound mode-only repair. ACL/xattr/capability/immutable/ownership editing and elevation remain excluded. |
...
| 18Y2 — Complete Undo and recovery | COMPLETE | `phase-18y2-complete-undo-recovery` | Private 30-day operation-specific persistent Undo/Redo for completed local copy, move, rename, and create work, with interruption review. | Verified exact raw paths, no-follow identities, no-overwrite inverse jobs, recoverable identity-checked Trash for copy/create Undo, empty-directory protection, bounded worker/storage, restart states, expiry, and UI review. Permanent delete, Undo Trash, Replace, and administrator mutations without complete GVfs post-commit identity evidence remain explicitly unavailable. |
...
| Phase | Status | Recommended branch | Scope | Dependencies; exclusions; acceptance |
| --- | --- | --- | --- | --- |
| 19A — Git awareness | NEXT | `phase-19a-git-awareness` | Cheap opt-in repository root, branch, status badges and relative-path actions. | 6T/11E; no cost outside repositories; verify large repos and missing Git. |
...
| 20B3 — Compact tabs | COMPLETE | `phase-20b3-compact-tabs` | Reduce tab-strip width and visual noise while preserving discoverability, accessibility, scrolling, drag/drop, and tab actions. | Verified 72px minimum/18-character centered titles, 24px label-strip height, single-line ellipsis/full-path tooltip and accessible description, hover/focus 20px labelled close action, underline-only active marker, neutral split boundary, and separate single-line device-name/free-space labels. Real-GTK and native Wayland gates pass. No multi-window or session architecture work. |

| 20C — Contextual help | COMPLETE | `phase-20c-contextual-help` | Explain complex controls and options through concise native hover help and equivalent accessible descriptions. | Central command descriptions feed action widgets and model-backed menus; 31 Settings Center entries retain visible subtitles and matching tooltips; explicit exact-path help remains authoritative; native full-control-tree and menu gates pass without custom menu replacement. |
...
## Phase 22 — File selection and desktop chooser service
...
and checksums. Sandboxed Preview depends on explicit provider and
