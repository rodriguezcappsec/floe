...
| Phase | Status | Recommended branch | Scope | Dependencies; exclusions; acceptance |
| --- | --- | --- | --- | --- |
| 22A — Floe Selection Mode | NEXT | `phase-selection-mode` | Native Open File(s), Select Folder, and Save File modes with explicit titles, accept labels, validation, cancellation, and exact local path results. | Reuse Floe browsing and selection architecture without requiring a portal. Keep filesystem work off GTK, preserve exact `PathBuf` identity, define a stable invocation/result contract, and test mode-specific keyboard and accessibility behavior. |
| 22B — Optional XDG FileChooser portal backend | PLANNED | `phase-xdg-filechooser-portal` | Optionally serve the freedesktop FileChooser backend contract through the verified Selection Mode UI. | 22A; isolate request handles, parent identifiers, response codes, cancellation, URI/document grants, and service availability. Floe remains independently usable when the portal backend is absent and must not imply sandbox permissions it did not grant. |
