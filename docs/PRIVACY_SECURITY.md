...
- Phase 10D Unix permission changes use typed exact targets and a capacity-4 application-owned executor. Direct and opt-in recursive requests complete a no-follow preflight before mutation, refuse roots and mount crossings, cap traversal at 250,000 entries and depth 1,024, revalidate device/inode identity, and distinguish pre-commit cancellation from committed partial failure. Numeric UID/GID values and validated local names are resolved by the worker through bounded no-follow local account-database reads. Floe does not collect credentials, log target paths or account names, follow symlinks, elevate its GTK process, or claim ACL/xattr/capability/immutable-bit coverage.
...
- Phase 6W records Undo only for a Floe-owned local GIO Trash action that yields exactly one new standards-correct payload/`.trashinfo` pair under one of at most eight reviewed local roots with at most 4,096 metadata entries. Exact raw paths and no-follow payload/metadata identities persist in the private 30-day Undo store. Restore revalidates both identities and never overwrites; Redo requires a fresh complete receipt. Missing, ambiguous, changed, malformed, unsafe, oversized, remote, or administrator receipts disable Undo without relabelling successful Trash as failed. This is not desktop-wide Trash history and never makes permanent deletion reversible.
...
- Bookmarks retain exact path bytes and use bounded asynchronous atomic persistence with `0700` directories and `0600` files. View preferences use asynchronous atomic `0600` file writes. Phase 20B2A adds only a bounded normal window width/height tuple; it stores no screen position, monitor, workspace, maximized/fullscreen state, path, or file metadata. Version 18 also stores color/click/font/motion choices, complete column order, and at most 32 validated collapsed-group labels. Extension group labels can reveal a file-type suffix to same-user configuration readers, but never reconstruct or replace exact entry paths.
...
- Phase 6P terminal history remains bounded and memory-only. Phase 18Y2 adds a separate private durable Undo/Redo store at `$XDG_STATE_HOME/floe/operation-undo-v1.bin` (or XDG fallback): at most 256 records and 4 MiB, with exact raw source/destination paths, operation recipe, no-follow file identity, timestamps, and Applied/Undone/interrupted state. Its parent is owner-only `0700`; atomic files are `0600`, no-follow opened, and completed history expires after 30 days. Phase 6U replacement records also retain an exact private backup path and both version identities. The history stores no hashes, passwords, keys, or administrator URIs and is not an audit log, snapshot, transaction, or general rollback system; replacement backups necessarily retain the old file contents until history expiry or explicit identity-checked resolution.
...
- Phase 12D template discovery examines at most 4,096 direct XDG Templates entries and returns at most 256 regular no-follow files on one application worker. Exact paths and raw names remain memory-only; the chooser never reads contents, executes templates, persists catalog/history, or reconstructs paths from lossy labels. Creation revalidates a no-follow destination descriptor and removes every execute bit from the copy without changing the source. This prevents an executable-permission surprise; it does not claim template contents are safe.
- Phase 12E symbolic-link planning is lexical and exact: the user chooses relative or absolute storage, Floe does not canonicalize or follow the target, and broken links remain permitted and explicitly described. Hard links accept only regular no-follow sources, preflight source/destination devices, publish without overwrite, and revalidate inode identity. Duplicate suffix parsing touches filenames only, remains bounded to 10,000 attempts, preserves raw bytes, and stores no history.
...
- Phase 6V completed-operation reveal holds at most 4,096 exact result paths in
  application memory, scoped to one job or batch and one visible directory
  generation. Reveal paths and the 1.8-second emphasis state are not written to
  preferences, history, logs, notifications, or a new cache. Durable operation
  history remains the separately documented Phase 18Y2 store; this visual
  reveal does not broaden its persistence or disclosure policy.
...
- Phase 6U replacement records extend the private durable Undo store with an exact backup path and both no-follow version identities. Replacement backups necessarily retain the old file contents until history expiry or explicit identity-checked resolution; they are not hashes, audit records, snapshots, or general rollback storage.
- Replace stages the incoming item beside the destination in owner-only `0700` `.floe-replace-backups`, capped at 64 entries, revalidates exact no-follow source and destination identities, and uses Linux `RENAME_EXCHANGE`. The old destination remains for Undo/Redo. Expiry and explicit resolution remove a backup only when its retained identity and private root still match; otherwise the record becomes review-required and the occupant is retained.
- Replacement is local-filesystem only. Unsupported filesystems fail without a fallback overwrite. Pre-commit cancellation or changed identities roll staging back; failed rollback or incomplete post-commit evidence is a partial result retained for review. Replace All is scoped to one batch, captures fresh identities for every conflict, and pauses at Protected Folder intersections for fresh review. Permanent deletion and administrator replacement remain unavailable; Phase 6W separately supports only exact-receipt Floe-owned local Trash Undo.
...
- Phase 18Y2 records local Copy, Move, Rename, and Create before mutation, then commits the exact destination identity only after success. Move/Rename Undo and Redo use no-overwrite inverse moves with fresh no-follow identity checks. Copy/Create Undo uses ordinary recoverable Trash; created directories must remain empty. Redo republishes without overwrite. Changed, missing, occupied, cancelled, or safely failed work retains its prior action state; partial or persistence-uncertain work becomes explicit Recovery Center review and is never cleaned automatically.
- Phase 6W adds durable Undo/Redo only for exact-receipt Floe-owned local Trash work. Permanent deletion remains unavailable. Administrator operations remain excluded because the current GVfs service boundary does not return a complete post-commit fingerprint or standards-correct Trash restore metadata; every administrator request obtains desktop authorization independently, but Floe does not claim a fresh-authorized inverse without evidence. Recursive administrator copy, ownership, ACL, and xattr changes are also excluded.
