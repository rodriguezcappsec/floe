...
- Permanent deletion and Undo Trash remain unavailable. Administrator operations remain excluded because the current GVfs service boundary does not return a complete post-commit fingerprint or standards-correct Trash restore metadata; every administrator request obtains desktop authorization independently, but Floe does not claim a fresh-authorized inverse without evidence. Recursive administrator copy, ownership, ACL, and xattr changes are also excluded.
