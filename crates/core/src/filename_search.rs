...
/// Searches names only and streams bounded batches to `on_batch`.
///
/// Child lookup failures are counted in the summary. The root itself remains
/// authoritative: an inaccessible or invalid root returns an error.
...
pub fn search_filenames_with_mime(
    ...
    while let Some((directory, depth)) = queue.pop_front() {
        ...
        for child in reader {
            ...
            if advanced_match {
                ...
                if batch.len() >= FILENAME_SEARCH_BATCH_CAPACITY {
                    if !on_batch(std::mem::take(&mut batch), summary) {
        ...
        if summary.examined_entries >= limits.entries || summary.matched >= limits.results {
            break;
        }
    }
...
mod tests {
    ...
    fn phase_13b_filename_search_preserves_raw_names_and_never_descends_symlinks() {
        ...
        assert!(
            entries
                .iter()
                .any(|entry| entry.display_name() == OsStr::new("linked-report"))
        );
        assert_eq!(
            entries
                .iter()
    ...
    #[test]
    fn phase_13b_filename_search_cancels_and_reports_explicit_caps() {
        let root = tempdir().expect("search fixture");
        for index in 0..8 {
        ...
        let limits = FilenameSearchLimits {
            results: 3,
            ..FilenameSearchLimits::default()
        };
        let (entries, summary) = collect(&request, limits).expect("capped search");
    ...
    #[test]
    fn phase_13b_filename_search_validates_query_root_and_limits() {
        assert!(matches!(
