...
mod tests {
    ...
    fn phase_13a_filter_matches_non_utf8_names_without_lossy_reconstruction() {
        ...
        assert!(
            FolderFilterPattern::compile_with_case(FolderFilterMode::Text, "Report", true)
                .expect("valid case-sensitive raw-name text filter")
                .matches(name)
        );
        assert!(
            !FolderFilterPattern::compile_with_case(FolderFilterMode::Text, "report", true)
                .expect("valid mismatched-case raw-name text filter")
                .matches(name)
        );
    }
}
